...
def get_excel1(user_name, pwd, time1, time2, medicaidProviderNum):
    ...
    data.columns = ["CatofServ", "recipiendId", "Recipient Name", "YOB", "Sex", "invoiceNumber", "priorApprovalNumber", "Item Code", "Item Code Mod", "serviceStarts", "Service Ends", "Approved To", "orderingProvider", "amt", "Qty", "Days/Times", "C D A"]
    data = data.drop([0])
    data = data.drop(columns="CatofServ")
    data['medicaidProviderNum'] = medicaidProviderNum
    # print(data)
...
if __name__ == "__main__":
    ...
    for x in result:
        OPTIONS[x['baseName']] = [x['userName'], x['password'], x['medicaidProviderNum']]
    ...
    def ok():
        baseName = variable.get()
        usrname = OPTIONS.get(baseName)[0]
        pwd = OPTIONS.get(baseName)[1]
        if baseName == "":
            messagebox.showinfo('Error','Choose a valid base')
            return
        # print(usrname, pwd)
        medicaidProviderNum = OPTIONS.get(baseName)[2]
        ok = get_excel1(usrname, pwd, time1.get(), time2.get(), medicaidProviderNum)
        if ok == True:
